...
class CallingHealthEndpoint extends BaseSampleTest {
  ...
  void should_return_application_name() {
    assertThat(response.path("components.app.details.name").toString()).isEqualTo("sample-service");
  }

  @Test
  void should_return_application_version() {
    assertThat(response.path("components.app.details.version").toString()).isNotBlank();
  }
